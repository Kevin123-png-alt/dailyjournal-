...
// Cek apakah (Rolenya Admin) ATAU (Usernamenya adalah danny)
if (
    (isset($_SESSION['username']) && $_SESSION['username'] === 'danny')|| 
    (isset($_SESSION['username']) && $_SESSION['username'] === 'admin')
) {
    // Jika salah satu syarat terpenuhi, izinkan masuk (kosongkan saja atau lanjut ke kode bawah)
} else {
    // Jika bukan admin DAN bukan danny, maka tendang keluar
    echo "<script>
        alert('Akses Ditolak! Hanya Admin dan Danny yang dapat mengakses halaman ini.');
...
<!-- Awal Modal Tambah-->
<div class="modal fade text-black" id="modalTambah" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            ...
            <form method="post" action="" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" placeholder="Tuliskan Password User" name="password" required></textarea>
                    </div>
                    <div class="mb-3">
...
if (isset($_POST['simpan'])) {
    ...
    if ($id) {
        ...
        if ($_POST['password'] == "") {
            // Jika input password kosong saat edit, gunakan password lama dari hidden input
            $password = $_POST['password_lama']; 
        } else {
            // Jika user mengisi password baru, enkripsi dengan MD5
            $password = md5($_POST['password']);
        }
    } else {
        // Logika untuk TAMBAH BARU (Wajib MD5)
        $password = md5($_POST['password']);
    }
